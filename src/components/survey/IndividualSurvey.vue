              ...
              <v-container fluid>
                <v-dialog v-model="Deletedialog" max-width="500px">
                  <v-card>
                    ...
                    <v-card-actions>
                      <v-btn color="error" @click="deleteThisSurvey" flat >Yes</v-btn>
                  ...
                  <v-card class="pl-2 pt-1 pb-4" >
                    <v-layout row wrap class="text-xs-center" v-if="isActive">
                      ...
                      <v-flex xs12 class="text-xs-right" v-show="view === true">
                        ...
                        <v-btn color="error" flat outline @click="deleteSurvey">Delete</v-btn>
                      </v-flex>
                      ...
                      <v-flex xs12 sm12 md12>
                        <div class="text-xs-left mb-3"><b>Attachments: </b>
                          <div v-for="(value, key) in data.attachments">
                            <a :href="value">{{value.filename}}</a>
                          </div>
                        </div>
                      </v-flex>
                    </v-layout>
                    <div row wrap v-else="isActive" v-show="adminUser">
                      <v-form ref="form">
                        ...
                        <v-layout row wrap>
                          <v-flex xs12 sm6 md4>
                            ...
                            min-width="290px"
                            >
                            <v-text-field slot="activator" v-model="data.start_date" label="From" prepend-icon="event" readonly></v-text-field>
                            <v-date-picker v-model="data.start_date" no-title scrollable>
                              <v-spacer></v-spacer>
                              <v-btn flat color="primary" @click="menu = false">Cancel</v-btn>
                              <v-btn flat color="primary" @click="$refs.menu[0].save(data.start_date)">OK</v-btn>
                            </v-date-picker>
                          </v-menu>
                        </v-flex>
                        <v-spacer></v-spacer>
                        <v-flex xs12 sm6 md4>
                          <v-dialog ref="dialog" v-model="modal" :return-value.sync="Todate" persistent lazy full-width width="290px">
                            <v-text-field slot="activator" v-model="data.end_date" label="To" prepend-icon="event" readonly></v-text-field>
                            <v-date-picker v-model="data.end_date" scrollable>
                              <v-spacer></v-spacer>
                              <v-btn flat color="primary" @click="modal = false">Cancel</v-btn>
                              <v-btn flat color="primary" @click="$refs.dialog[0].save(data.end_date)">OK</v-btn>
                            </v-date-picker>
                          </v-dialog>
                        </v-flex>
                      </v-layout>
                      ...
                      <v-flex xs12 md6 sm12 class="pr-3">
                        <input type="file" id="files" ref="files" v-show="show=false" v-on:change="handleFilesUpload()"/>
...
export default{
  ...
  mounted:function(){
    ...
    .then(response =>{
      ...
      this.attach = survey.attachments
      if(survey.survey_image === null){
          this.photo = 'https://upload.wikimedia.org/wikipedia/commons/thumb/a/ac/No_image_available.svg/1024px-No_image_available.svg.png'
      } else {
          this.photo = survey.survey_image
      }
  ...
  methods:{
    ...
    deleteThisAttachment(){
      ...
      var obj = this
      axios({
        url: this.$hostname + '/survey/surveys/'+localStorage.getItem('survey_id')+'/attachment?id='+obj.attachmentId,
        method: 'delete',
    ...
    deleteThisSurvey () {
      var obj = this
      axios({
        url: obj.$hostname + '/survey/surveys/' + localStorage.getItem('survey_id') + '/',
        method: 'delete',
        headers: {'Authorization': 'Token ' + localStorage.getItem('api_token') },
      }).then(function(res) {
    ...
    handleFilesUpload(){
      let uploadedFiles = this.$refs.files[0].files;
    ...
    submit () {
      ...
      form.append("survey_custom_id", survey.surveyId)
      form.append("survey_name", survey.surveyName)
      form.append("no_of_participants", survey.participants)
      form.append("country", survey.country)
      form.append("notes", survey.notes)
      form.append("no_of_questions", survey.questions)
      form.append("target_population", survey.targetPopulation)
